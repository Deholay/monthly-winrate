...
INDEXES = {
    "IXIC": {"symbol": "^IXIC", "title": "NASDAQ Composite"},
    "SOX": {"symbol": "^SOX", "title": "PHLX Semiconductor Index"},
    "SP500": {"symbol": "^GSPC", "title": "S&P 500"},
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch Yahoo Finance index close history and draw 12 monthly overlays "
    ...
    parser.add_argument(
        ...
        default=sorted(INDEXES),
        help="Index keys to process. Defaults to all supported indexes.",
    )
    parser.add_argument(
        "--start-year",
        type=int,
        default=DEFAULT_START_YEAR,
        help="First year to plot. Defaults to current year minus 9.",
    )
    parser.add_argument(
        "--end-year",
        type=int,
        default=DEFAULT_END_YEAR,
        help="Last year to plot. Defaults to current year.",
    )
    parser.add_argument(
        "--cache-root",
        default="outputs/cache/yahoo_chart",
        help="Root folder for Yahoo Finance chart JSON cache inside each index folder.",
    )
    parser.add_argument(
        "--no-cache",
        action="store_true",
        help="Always fetch from Yahoo Finance even when cached JSON exists.",
    )
    return parser.parse_args()


def year_start_timestamp(year: int) -> int:
    return int(pd.Timestamp(year=year, month=1, day=1, tz="UTC").timestamp())


def year_end_timestamp(year: int) -> int:
    return int(
        (
...
def load_yahoo_close_history(
    ...
    fetch_start = pd.Period(year=start_year, month=1, freq="M")
    if include_previous_month:
        fetch_start -= 1
    period1 = int(fetch_start.start_time.tz_localize("UTC").timestamp())
    period2 = year_end_timestamp(end_year)
    payload = fetch_yahoo_chart(symbol, period1, period2, cache_path, use_cache=use_cache)
    ...
    if not result:
        raise RuntimeError(f"No Yahoo Finance chart rows found for {symbol}.")
    ...
    for timestamp, close in zip(timestamps, closes):
        if close is None:
            continue
        trade_date = pd.to_datetime(timestamp, unit="s", utc=True).tz_convert(None).normalize()
        if trade_date.year > end_year:
            continue
        records.append({"trade_date": trade_date, "close": float(close)})

    if not records:
        raise RuntimeError(f"No valid close rows found for {symbol}.")
...
def plot_monthly_year_overlay(
    ...
    fig.suptitle(
        f"{years[0]}-{years[-1]} {title} 每月收盤報酬疊圖",
        fontsize=18,
        fontweight="bold",
        y=0.995,
    )
    ...
    fig.supylabel("相對上月底收盤報酬 (%)", fontsize=11)
    fig.text(
        0.5,
        0.012,
...
def process_index(index_key: str, start_year: int, end_year: int, cache_root: str, use_cache: bool) -> None:
    index = INDEXES[index_key]
    base_dir = Path(index_key)
    outputs_dir = base_dir / "outputs"
    cache_path = base_dir / cache_root / f"{index_key}_{start_year - 1}_{end_year}.json"
    output = outputs_dir / f"{index_key.lower()}_10y_monthly_close_overlay.png"
    returns_output = outputs_dir / f"{index_key.lower()}_10y_monthly_returns.csv"
    summary_output = outputs_dir / f"{index_key.lower()}_10y_calendar_month_summary.csv"

    data = load_yahoo_close_history(
        index["symbol"],
        start_year,
        end_year,
        cache_path,
    ...
    print(f"{index_key}: wrote {output.resolve()}")
    print(f"{index_key}: wrote {returns_output.resolve()}")
    print(f"{index_key}: wrote {summary_output.resolve()}")
    print(f"{index_key}: rows {len(data):,}; years {start_year}-{end_year}")
